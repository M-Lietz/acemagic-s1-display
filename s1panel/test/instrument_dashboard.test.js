...
test('animates changed values over time and keeps a smoothed five-minute history', async () => {
    const canvas = createCanvas(170, 320);
    const context = canvas.getContext('2d');
    let now = 10000;
    const config = {
    ...
    assert.equal(config._private.displayedMetrics.cpuPercent, 6);
    now += 500;
    assert.equal(await dashboard.draw(context, changed, 0, 0, config), true);
    assert.ok(config._private.displayedMetrics.cpuPercent > 6);
    assert.ok(config._private.displayedMetrics.cpuPercent < 66);
